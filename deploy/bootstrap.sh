...
INGRESS_HOSTS=(
  "$(yq e '.airflow.hostname'          "$PROJECT_ROOT/config/env-config.yaml")"
  "$(yq e '.jupyterhub.hostname'        "$PROJECT_ROOT/config/env-config.yaml")"
  "$(yq e '.minio.hostname'            "$PROJECT_ROOT/config/env-config.yaml")"
...
echo ""
echo "portless service URLs (only on the machine running 'bash deploy/dev-proxy.sh'):"
...
echo ""
echo "Host aliases written on this server only (/etc/hosts -> $NODE_IP):"
echo "  airflow.dakar-datasphere-node.local   — Airflow ingress hostname"
echo "  jupyter.dakar-datasphere-node.local   — JupyterHub ingress hostname"
echo "  minio.dakar-datasphere-node.local     — MinIO S3/API ingress hostname"
echo "  grafana.dakar-datasphere-node.local   — Grafana ingress hostname"
echo ""
echo "If you want to browse from another computer on the same network,"
echo "add matching host entries on that computer as well."
echo "These hostnames assume ingress-nginx is reachable on the node IP via k3s ServiceLB."
echo ""
